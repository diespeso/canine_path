...
router.post('/perros', (req, res) => {
    ...
    con.connect();
    con.query('USE canine_path;');
    con.query(`SELECT * FROM perro;`, function(err, rows, fields) {
        if(err) throw err;
        var result = "";
        for(var i = 0; i < rows.length; i++) {
            result += `<a href="perro/${rows[i].id}" class="dog_container_clicker"><div class="dog_container">
            <img class="dog_pic" src="/img/dog_profiles/${rows[i].id}.jpg">
            <h1>${rows[i].name}</h1>
        </div></a>`;
        }
        res.render('buscador_perros', {
...
router.get('/refugio/:id', (req, res) => {
    ...
    var con = mysql.createConnection(config.db_con);
    con.query('use canine_path;');
    /*con.query(`select *, perro.id as perro_id, perro.name as perro_name
        from refugio
        inner join perro
            on refugio.id = perro.id_refugio
        where refugio.id = ${req.params.id};`, (err, rows, fields) => {
            console.log(rows);
